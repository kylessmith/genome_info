...
SHORTDESC = "Genome information class"
...
ext_module_kmer = declare_cython_extension( "genome_info.kmers.kmer_reader", use_math=False, use_openmp=False , include_dirs=my_include_dirs )
...
setup(
    ...
    url = "https://github.com/kylessmtih/genome_info",
    ...
    provides = ["genome_info"],
    keywords = ["genome", "info"],
    ext_modules = my_ext_modules,
    packages = find_namespace_packages(include=["genome_info", "genome_info.*"]),
    package_data={'genome_info': ['*.pxd', '*.pyx', '*.c', '*.h', '*.h5', '*.bed.gz', '*.parquet', '*.pfm'],
                  '': ['*.pxd', '*.pyx', '*.c', '*.h', '*.h5', '*.bed.gz', '*.parquet','*.pfm']},
    # Disable zip_safe
    zip_safe = False,
